...
def muscle_length_function(muscle_length,muscle_velocity,step,Lmt,L0T,Lse,Lce,Vce,U_eff,f_eff_slow,f_eff_fast,W_slow,W_fast,Y,S,Lmax,F0,L0,mass,alpha):    
   
    # k_0_de = step*muscle_velocity;
    # l_0_de = step*contraction_dynamics(Lse,Lce,Vce,U_eff,f_eff_slow,f_eff_fast,W_slow,W_fast,Y,S,step,Lmax,F0,L0,mass,alpha);
    # k_1_de = step*(muscle_velocity+l_0_de/2);
    # l_1_de = step*contraction_dynamics((Lmt - (Lce+k_0_de/L0)*L0*np.cos(alpha))/L0T,Lce+k_0_de/L0,Vce+l_0_de/L0,U_eff,f_eff_slow,f_eff_fast,W_slow,W_fast,Y,S,step,Lmax,F0,L0,mass,alpha);
    # k_2_de = step*(muscle_velocity+l_1_de/2);
    # l_2_de = step*contraction_dynamics((Lmt - (Lce+k_1_de/L0)*L0*np.cos(alpha))/L0T,Lce+k_1_de/L0,Vce+l_1_de/L0,U_eff,f_eff_slow,f_eff_fast,W_slow,W_fast,Y,S,step,Lmax,F0,L0,mass,alpha);
    # k_3_de = step*(muscle_velocity+l_2_de);
    # l_3_de = step*contraction_dynamics((Lmt - (Lce+k_2_de/L0)*L0*np.cos(alpha))/L0T,Lce+k_2_de/L0,Vce+l_2_de/L0,U_eff,f_eff_slow,f_eff_fast,W_slow,W_fast,Y,S,step,Lmax,F0,L0,mass,alpha);
    # muscle_length = muscle_length + 1/6*(k_0_de+2*k_1_de+2*k_2_de+k_3_de);
    # muscle_velocity = muscle_velocity + 1/6*(l_0_de+2*l_1_de+2*l_2_de+l_3_de);
    
    muscle_acc = contraction_dynamics(Lse,Lce,Vce,U_eff,f_eff_slow,f_eff_fast,W_slow,W_fast,Y,S,step,Lmax,F0,L0,mass,alpha)
    muscle_velocity = muscle_acc*step + muscle_velocity
    muscle_length = muscle_velocity*step + muscle_length
    Vce = muscle_velocity/float(L0/100)
    Lce = muscle_length/float(L0/100)
    
    return (muscle_length,muscle_velocity,Lce,Vce)